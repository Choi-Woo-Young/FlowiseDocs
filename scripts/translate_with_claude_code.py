#!/usr/bin/env python3
"""
Translate Embeddings and LLMs files using Claude Code
Falls back to direct API if credentials are available
"""
...
# The files to translate: embeddings (13) + llms (8)
EMBEDDINGS_FILES = [
...
def translate_content_with_api(content: str, file_path: str) -> Optional[str]:
    """Try to translate using the Anthropic API directly."""
    try:
        import anthropic
...
def process_files() -> Tuple[int, int, List[Tuple[str, str]]]:
    ...
    for idx, rel_path in enumerate(FILES_TO_TRANSLATE, 1):
        try:
            ...
            if not en_file.exists():
                print(f" SKIP (EN file not found)")
                failed += 1
            ...
            # Translate content
            translated_content = translate_content_with_api(content, str(en_file))

            if translated_content is None:
                raise Exception("Translation failed - API not available")
...
def main():
    ...
    # Process the files
    successful, failed, failed_files = process_files()
