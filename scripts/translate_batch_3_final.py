#!/usr/bin/env python3
"""
FlowiseDocs Batch 3 Final Translation Script
...
def translate_file(file_path):
    ...
    try:
        ...
        if contains_korean(translated_body):
            return True
        else:
            print(f"    WARNING: No Korean text detected")
            return True  # Still count as translated
    ...
    for index, file_path in enumerate(FILES_TO_TRANSLATE, 1):
        ...
        try:
            if translate_file(file_path):
                print(f"[{index:2d}/{total_files}] {filename:40s} ✓ translated")
                translated_count += 1
            else:
                print(f"[{index:2d}/{total_files}] {filename:40s} ✗ FAILED")
                failed_files.append(file_path)
        except Exception as e:
            print(f"[{index:2d}/{total_files}] {filename:40s} ✗ ERROR: {str(e)[:50]}")
            failed_files.append(file_path)
