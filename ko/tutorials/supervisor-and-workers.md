...
* **소프트웨어 Engineer**: Specialized in designing and implementing software solutions
...
1. 추가 a **Start** node to your canvas
2. Configure the **Input 타입** as "Chat Input"
3. 집합 up **흐름 상태** with these initial variables:
...
4. 집합 up **JSON Structured 출력** with these fields:
...
5. Configure **업데이트 흐름 상태** to store:
   * `next`: `{{ output.next }}`
   * `instruction`: `{{ output.instructions }}`
6. 집합 the **Input 메시지** to: _"Given the conversation above, who should act next? Or should we FINISH? Select one of: SOFTWARE, REVIEWER."_ The Input 메시지 will be inserted at the end, as if the user is asking the supervisor to assign the next agent.

<figure><img src="../.gitbook/assets/Untitled-2025-06-19-1011.png" alt="" width="563"><figcaption></figcaption></figure>

### Step 3: Create the Routing 조건
...
1. 추가 a **조건** node after the Supervisor
2. 집합 up two conditions:
   * **조건 0**: `{{ $flow.state.next }}` equals "SOFTWARE"
   * **조건 1**: `{{ $flow.state.next }}` equals "REVIEWER"
3. The "Else" branch (조건 2) will handle the "FINISH" case
...
### Step 4: Configure the 소프트웨어 Engineer Agent

<figure><img src="../.gitbook/assets/image (12).png" alt="" width="296"><figcaption></figcaption></figure>

The **소프트웨어 Engineer** specializes in designing and implementing software solutions.

1. Connect an **Agent** node to 조건 0 output
...
4. 집합 **Input 메시지** to: `{{ $flow.state.instruction }}` . The Input 메시지 will be inserted at the end, as if the user is giving an instruction to the 소프트웨어 Engineer Agent.
...
1. Connect an **Agent** node to 조건 1 output
2. Configure the system message:
...
4. 집합 **Input 메시지** to: `{{ $flow.state.instruction }}` . The Input 메시지 will be inserted at the end, as if the user is giving an instruction to the Code Reviewer Agent.

<figure><img src="../.gitbook/assets/image (15).png" alt="" width="563"><figcaption></figcaption></figure>

### Step 6: 추가 루프 Back Connections

<figure><img src="../.gitbook/assets/image (17).png" alt="" width="563"><figcaption></figcaption></figure>

Both worker agents need to loop back to the Supervisor for continued coordination.

1. 추가 a **루프** node after the 소프트웨어 Engineer
   * 집합 **루프 Back To** as "Supervisor"
   * 집합 **Max 루프 Count** to 5
2. 추가 another **루프** node after the Code Reviewer
   * 집합 **루프 Back To** as "Supervisor"
   * 집합 **Max 루프 Count** to 5
...
1. Connect an **Agent** node to 조건 2 output (the "Else" branch)
...
3. 집합 **Input 메시지.** This is important because Input 메시지 will be inserted at the end, as if the user is giving an instruction to the Final Answer Generator to look at all the conversations, and generate a final response.
...
**Intelligent 작업 Delegation**: The supervisor uses context and reasoning to assign the most appropriate worker for each sub-task.
...
1. **사용자**: "Create a React component for user authentication with form validation"
2. **Supervisor**: Decides SOFTWARE should act first to implement the component
3. **소프트웨어 Engineer**: Creates a React authentication component with validation logic
...
7. **소프트웨어 Engineer**: Updates the component based on feedback
...
* This architecture consumes a lot of tokens due to the back and forth communications between agents, hence it is not suitable for every cases. It is particularly effective for:
  * 소프트웨어 development tasks requiring both implementation and review
...
* Use memory settings appropriately to maintain conversation context while avoiding token limit issues. Consider using memory optimization settings like "Conversation 요약 버퍼" for longer workflows.
