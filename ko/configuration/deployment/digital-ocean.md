...
In this section, we are going to create a Droplet. For more information, refer to [official 가이드](https://docs.digitalocean.com/products/droplets/quickstart/).
...
3. Select 인증 Method. In this 예시, we are going to use Password
...
For Windows follow this [가이드](https://docs.digitalocean.com/products/droplets/how-to/connect-with-ssh/putty/).

For Mac/Linux, follow this [가이드](https://docs.digitalocean.com/products/droplets/how-to/connect-with-ssh/openssh/).
...
7. You can then view the app: "Your Public IPv4 DNS":3000. 예시: `176.63.19.226:3000`
...
1. Nginx is available for 설치 with apt through the default repositories. Update your repository index, then install Nginx:
...
> Press Y to confirm the 설치. If you are asked to restart services, press ENTER to accept the defaults.

2. You need to allow access to Nginx through your firewall. Having set up your server according to the initial server 사전 요구사항, add the following rule with ufw:
...
It is recommended practice to create a custom 설정 file for your new server block additions, instead of editing the default 설정 directly.

1. Create and open a new Nginx 설정 file using nano or your preferred text editor:
...
4. Next, enable this 설정 file by creating a link from it to the sites-enabled directory that Nginx reads at startup, making sure again to replace `your_domain` with your own domain name::

```bash
sudo ln -s /etc/nginx/sites-available/your_domain /etc/nginx/sites-enabled/
```

5. You can now test your 설정 file for syntax errors:
...
4. Finally, run the following command to obtain a certificate and let Certbot automatically modify the NGINX 설정, enabling HTTPS:
...
You have successfully 설정 Flowise on your Droplet, with SSL certificate on your domain [🥳](https://emojipedia.org/partying-face/)
