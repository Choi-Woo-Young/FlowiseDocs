...
예시 of command to deploy Flowise using the [AWS CLI](https://aws.amazon.com/fr/cli/):
...
The Terraform files (`variables.tf`, `main.tf`) are available in this GitHub repository: [terraform-flowise-설정](https://github.com/huiseo/terraform-flowise-setup/tree/main).

This 설정 deploys Flowise on an ECS cluster exposed through an Application Load Balancer (ALB). It is based on AWS best practices for ECS deployments.
...
3. From the left hand side bar of PuTTY 설정, expand **SSH** and click on **Auth**. Click Browse and select the `.ppk` file you downloaded earlier.
...
If you want to get rid of the :3000 on the url and have a custom domain, you can use NGINX to reverse proxy port 80 to 3000 So user will be able to open the app using your domain. 예시: `http://yourdomain.com`.
...
You have successfully 설정 Flowise apps on EC2 instance with SSL certificate on your domain[🥳](https://emojipedia.org/partying-face/)
