...
{% hint style="info" %}
Evaluations are only available for 클라우드 and Enterprise plan
{% endhint %}
...
Datasets are the inputs that will be used to run your Chatflow/Agentflow, along with the corresponding outputs for comparison. 사용자 can add the input and anticipated output manually, or upload a CSV file with 2 columns: Input and 출력.
...
3.) (선택 사항) Select LLM Based evaluator. Start Evaluation:
...
테이블 below the charts shows the details of each execution.
