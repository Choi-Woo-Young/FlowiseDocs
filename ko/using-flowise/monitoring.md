...
Flowise has native support for Prometheus with Grafana and OpenTelemetry. However, only high-level metrics such as API requests, counts of flows/predictions are tracked. Refer [here](https://github.com/FlowiseAI/Flowise/blob/main/packages/server/src/인터페이스.메트릭.ts#L13) for the lists of counter metrics. For details node by node observability, we recommend using [Analytic](/broken/pages/z1V6RsbL6q6hrrswC3e9).
...
2. 저장 the API key to a file accessible by Prometheus (e.g., `/etc/prometheus/api_key.txt`)
...
On the left side bar, click 추가 new connection, and select Prometheus:
...
Now that connection is added successfully, we can start adding dashboard. From the left side bar, click Dashboards, and Create 대시보드.
