...
예측 API is the primary endpoint for interacting with your Flowise flows and assistants. It allows you to send messages to your selected flow and receive responses back. This API handles the core chat functionality, including:
...
* **Conversation 메모리**: Maintain context across multiple messages within a session
* **File Processing**: 업로드 and process images, audio, and other files as part of your queries
* **Dynamic 구성**: 오버라이드 chatflow settings and pass variables at runtime

For details, see the [예측 Endpoint API 참조](../api-reference/prediction.md).
...
| `overrideConfig` | object  | No                          | 오버라이드 flow configuration                 |
...
You can override the value by 변수 Name of the Form Input
...
오버라이드 chatflow settings dynamically.

오버라이드 config is **disabled** by default for security reasons. Enable it from the top right: **Settings** → **구성** → **보안** tab:
...
배열 value from overrideConfig will concatenate with existing array values. For example, if existing `startState` has:
...
From the 실행, you can see the overriden system message:
...
If you head back to 실행, you can see each LLM has the correct overriden value:
...
업로드 images for visual analysis when your flow supports image processing. Refer to [Image](uploads.md#image) for more reference.
...
업로드 audio files for speech-to-text processing. Refer to [Audio](uploads.md#audio) for more reference.
...
업로드 files to have LLM process the files and answer query related to the files. Refer to [Files](uploads.md#files) for more reference.
...
| `feedback`    |                           | 선택 사항 feedback text if `humanInputEnableFeedback` was true              |

**예제 — reject with feedback:**
...
3. **400 Bad 요청**: Check request format and required fields
4. **413 Payload Too Large**: Reduce file sizes or split large requests
5. **500 Internal 서버 오류:** Check if there is any misconfiguration from the nodes in the flow
