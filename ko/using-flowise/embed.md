...
* **버튼:** Position, size, color, icon, drag-and-drop behavior, and automatic opening.
...
* **Chat 창:** Title, agent/user message display, welcome/error messages, background color/image, dimensions, font size, starter prompts, HTML rendering, message styling (colors, avatars), text input behavior (placeholder, colors, character limits, sounds), feedback options, date/time display, and footer customization.
...
**참고:** See full [configuration list](https://github.com/FlowiseAI/FlowiseChatEmbed#configuration)
...
5. 푸시 changes to the forked repository
6. You can then use your custom `web.js` as embedded chat like so:

교체 `username` to your Github username, and `forked-repo` to your forked repo.
...
{% hint style="danger" %}
Access to fetch at 'https://\<your-flowise.com>/api/v1/prediction/' from origin 'https://\<your-flowise.com>' has been blocked by CORS policy: No 'Access-제어-Allow-원본' header is present on the requested resource.
{% endhint %}
